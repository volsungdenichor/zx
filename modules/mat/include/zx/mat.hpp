...
struct dot_fn
{
    ...
    constexpr auto operator()(const vector_t<T, D>& lhs, const vector_t<U, D>& rhs) const -> Res
    {
...
struct cross_fn
{
    ...
    constexpr auto operator()(const vector_t<T, 2>& lhs, const vector_t<U, 2>& rhs) const -> Res
    {
        return lhs[0] * rhs[1] - lhs[1] * rhs[0];
    }

    template <class T, class U, class Res = std::invoke_result_t<std::multiplies<>, T, U>>
    constexpr auto operator()(const vector_t<T, 3>& lhs, const vector_t<U, 3>& rhs) const -> vector_t<Res, 3>
    {
        return vector_t<Res, 3>{ { lhs[1] * rhs[2] - lhs[2] * rhs[1],  //
...
{
    template <class T>
    constexpr auto operator()(const vector_t<T, 2>& lhs, const vector_t<T, 2>& rhs) const
    ...
    template <class T>
    constexpr auto operator()(const vector_t<T, 3>& lhs, const vector_t<T, 3>& rhs) const
...
struct norm_fn
{
    ...
    constexpr auto operator()(const vector_t<T, D>& item) const -> Res
    {
...
{
    template <class T, std::size_t D>
    constexpr auto operator()(const vector_t<T, D>& item) const -> decltype(math::sqrt(norm(item)))
    {
...
struct unit_fn
{
    ...
    constexpr auto operator()(const vector_t<T, D>& item) const -> vector_t<Res, D>
    {
...
struct distance_fn
{
    ...
    constexpr auto operator()(const vector_t<T, D>& lhs, const vector_t<U, D>& rhs) const -> decltype(length(rhs - lhs))
    {
...
struct center_fn
{
    ...
    template <class T, std::size_t D>
    constexpr auto operator()(const segment_t<T, D>& item) const -> vector_t<T, D>
    {
        return (item[0] + item[1]) / 2;
    }

    template <class T, std::size_t D>
    constexpr auto operator()(const spherical_shape_t<T, D>& item) const -> vector_t<T, D>
    {
        return item.center;
    }
};
...
struct orientation_fn
{
    ...
    constexpr auto operator()(const vector_t<T, 2>& point, const vector_t<U, 2>& start, const vector_t<U, 2>& end) const
    {
        return cross(end - start, point - start);
    }

    template <class T, class U, class Tag>
    constexpr auto operator()(const vector_t<T, 2>& point, const linear_shape_t<Tag, U, 2>& shape) const
    {
...
struct contains_fn
{
    ...
    template <class T, class U>
    constexpr bool operator()(const triangle_t<T, 2>& item, const vector_t<U, 2>& other) const
    {
...
struct interpolate_fn
{
    ...
    constexpr auto operator()(R r, const vector_t<T, D>& lhs, const vector_t<T, D>& rhs) const -> vector_t<T, D>
    {
...
constexpr auto get_line_intersection_parameter(
    const vector_t<T, 2>& a0, const vector_t<T, 2>& a1, const vector_t<T, 2>& p, E epsilon) -> std::optional<T>
{
...
constexpr auto get_line_intersection_parameters(
    const vector_t<T, 2>& a0, const vector_t<T, 2>& a1, const vector_t<T, 2>& b0, const vector_t<T, 2>& b1, E epsilon)
...
struct intersection_fn
{
    ...
    constexpr auto operator()(const linear_shape_t<Tag1, T, D>& lhs, const linear_shape_t<Tag2, T, D>& rhs, E epsilon = {})
        const -> std::optional<vector_t<T, D>>
    {
...
{
    template <class T, std::size_t D>
    constexpr auto operator()(const vector_t<T, D>& lhs, const vector_t<T, D>& rhs) const
    ...
    constexpr auto operator()(const vector_t<T, D>& point, const linear_shape_t<Tag, T, D>& shape, E epsilon = {}) const
        -> std::optional<vector_t<T, D>>
    {
...
{
    template <class T>
    constexpr auto operator()(const vector_t<T, 2>& value) const -> vector_t<T, 2>
    {
        return vector_t<T, 2>{ -value[1], value[0] };
    }

    template <class Tag, class T>
    constexpr auto operator()(const linear_shape_t<Tag, T, 2>& value, const vector_t<T, 2>& origin) const
...
{
    template <typename T>
    constexpr auto operator()(const triangle_t<T, 2>& value) const -> vector_t<T, 2>
    {
        return std::accumulate(std::begin(value), std::end(value), vector_t<T, 2>{}) / 3;
    }
};
