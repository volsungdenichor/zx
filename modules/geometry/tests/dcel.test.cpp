...
TEST(dcel, add_vertex)
{
    ...
    const auto v0 = dcel.m_vertices[0].id;
    const auto v1 = dcel.m_vertices[1].id;
    const auto v2 = dcel.m_vertices[2].id;
    const auto v3 = dcel.m_vertices[3].id;

    dcel.m_faces.push_back({ 0, 0 });
    dcel.m_faces.push_back({ 1, 6 });

    dcel.m_halfedges.push_back({ 0, v0, 1, 2, 4, 0 });
    dcel.m_halfedges.push_back({ 1, v1, 0, 7, 3, -1 });
    dcel.m_halfedges.push_back({ 2, v1, 3, 4, 0, 0 });
    dcel.m_halfedges.push_back({ 3, v2, 2, 1, 9, -1 });
    dcel.m_halfedges.push_back({ 4, v2, 5, 0, 2, 0 });
    dcel.m_halfedges.push_back({ 5, v0, 4, 8, 6, 1 });
    dcel.m_halfedges.push_back({ 6, v3, 7, 5, 8, 1 });
    dcel.m_halfedges.push_back({ 7, v0, 6, 9, 1, -1 });
    dcel.m_halfedges.push_back({ 8, v2, 9, 6, 5, 1 });
    dcel.m_halfedges.push_back({ 9, v3, 8, 3, 7, -1 });
    ...
    EXPECT_THAT(
        dcel.vertices(),
        testing::ElementsAre(
            testing::AllOf(IdIs(0), testing::Property(&D::vertex_t::location, zx::mat::vector(648, 112))),
            testing::AllOf(IdIs(1), testing::Property(&D::vertex_t::location, zx::mat::vector(385, 147))),
            testing::AllOf(IdIs(2), testing::Property(&D::vertex_t::location, zx::mat::vector(459, 303))),
            testing::AllOf(IdIs(3), testing::Property(&D::vertex_t::location, zx::mat::vector(656, 330)))));

    EXPECT_THAT(
        dcel.faces(),
        testing::ElementsAre(
            testing::AllOf(IdIs(0), testing::Property(&D::face_t::halfedge, IdIs(0))),
            testing::AllOf(IdIs(1), testing::Property(&D::face_t::halfedge, IdIs(6)))));
