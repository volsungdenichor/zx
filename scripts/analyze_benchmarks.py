...
def extract_operation_name(benchmark_name: str) -> str:
    """Extract operation name from benchmark name"""
    # e.g., "BM_Template_RangeIteration" -> "RangeIteration"
    parts = benchmark_name.split('_')
    if len(parts) >= 3:
        return parts[2]
    return benchmark_name

def extract_approach(benchmark_name: str) -> str:
    """Extract approach type from benchmark name"""
    # e.g., "BM_Template_RangeIteration" -> "Template"
    parts = benchmark_name.split('_')
    if len(parts) >= 2:
        return parts[1]
    return "Unknown"
...
def group_benchmarks(benchmarks: List[Dict]) -> Dict[str, Dict[str, List[Dict]]]:
    """Group benchmarks by operation and approach"""
    grouped = {}
    for bench in benchmarks:
        op = extract_operation_name(bench['name'])
        approach = extract_approach(bench['name'])

        if op not in grouped:
            grouped[op] = {}
        if approach not in grouped[op]:
            grouped[op][approach] = []

        grouped[op][approach].append(bench)
...
def compare_approaches(template_benches: List[Dict], erased_benches: List[Dict]) -> Dict:
    """Compare template and erased benchmarks"""
    ...
    return {
        'template': t_metrics,
        'erased': e_metrics,
        'ratio': ratio,  # How many times slower is erased
        'improvement_pct': improvement_pct,  # Percentage improvement of template
    }
...
def print_comparison_table(grouped: Dict[str, Dict[str, List[Dict]]]):
    ...
    for op in sorted(grouped.keys()):
        ...
        if 'Template' in approaches and 'Erased' in approaches:
            comparison = compare_approaches(approaches['Template'], approaches['Erased'])
            ...
            print(f"{op:<20} {t_mean:>13.2f} {e_mean:>15.2f} {ratio:>8.2f}x {improvement:>12.1f}%")
...
def print_insights(grouped: Dict[str, Dict[str, List[Dict]]]):
    ...
    if ratios:
        ...
        for op, ratio in ratios[:3]:
            print(f"  1. {op:<20} {ratio:>6.2f}x slower (type-erased)")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python scripts/analyze_benchmarks.py <JSON_results_file> [--detailed] [--insights]")
        print("\nExample:")
        print("  python scripts/analyze_benchmarks.py results.json")
        print("  python scripts/analyze_benchmarks.py results.json --detailed --insights")
        sys.exit(1)
