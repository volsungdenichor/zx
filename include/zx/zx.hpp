...
struct pipeable
{
    ...
    template <class Arg>
    constexpr friend auto operator>>=(const pipeable& item, Arg&& arg) -> std::invoke_result_t<Func, Arg>
    {
        return item(std::forward<Arg>(arg));
    }
};
...
struct transform_fn
{
    template <bool, class Func, class Next>
    struct reducer;
    template <class Func, class Next>
    struct reducer<false, Func, Next>
    {
        Func m_func;
        Next m_next;
    ...
    struct transducer
    {
        ...
        template <class Next>
        constexpr auto operator()(Next&& next) const& -> reducer<Indexed, Func, std::decay_t<Next>>
        {
            return { m_func, std::forward<Next>(next) };
        }

        template <class Next>
        constexpr auto operator()(Next&& next) && -> reducer<Indexed, Func, std::decay_t<Next>>
        {
            return { std::move(m_func), std::forward<Next>(next) };
        }
    };

    template <class Func>
    constexpr auto operator()(Func&& func) const -> pipeable<transducer<std::decay_t<Func>>>
    {
        return { { std::forward<Func>(func) } };
    }
};
...
struct filter_fn
{
    template <bool, class Pred, class Next>
    struct reducer;

    template <class Pred, class Next>
    struct reducer<false, Pred, Next>
    {
        Pred m_pred;
        Next m_next;
        ...
        constexpr auto operator()(State state, Args&&... args) const -> State
        {
            return std::invoke(m_pred, args...) ? m_next(std::move(state), std::forward<Args>(args)...) : state;
        }
    };

    template <class Pred, class Next>
    struct reducer<true, Pred, Next>
    {
        Pred m_pred;
        Next m_next;
        mutable std::ptrdiff_t m_index = 0;
        ...
        constexpr auto operator()(State state, Args&&... args) const -> State
        {
            return std::invoke(m_pred, m_index++, args...) ? m_next(std::move(state), std::forward<Args>(args)...) : state;
        }
    };
    ...
    struct transducer
    {
        ...
        template <class Next>
        constexpr auto operator()(Next&& next) const& -> reducer<Indexed, Pred, std::decay_t<Next>>
        {
            return { m_pred, std::forward<Next>(next) };
        }

        template <class Next>
        constexpr auto operator()(Next&& next) && -> reducer<Indexed, Pred, std::decay_t<Next>>
        {
            return { std::move(m_pred), std::forward<Next>(next) };
        }
    };

    template <class Pred>
    constexpr auto operator()(Pred&& pred) const -> pipeable<transducer<std::decay_t<Pred>>>
    {
...
struct yield_fn
{
    ...
    constexpr auto operator()(Out out, Arg&& arg) const -> Out
    {
...
static constexpr inline auto transform_maybe_indexed = detail::transform_maybe_fn<true>{};
...
static constexpr inline auto inspect_indexed = detail::inspect_fn<true>{};
static constexpr inline auto yield = detail::yield_fn{};
