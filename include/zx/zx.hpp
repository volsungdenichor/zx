...
template <typename T>
constexpr bool should_pass_by_value = sizeof(T) <= 2 * sizeof(void *) &&
...
template <class T>
using in_t =
    std::conditional_t<detail::should_pass_by_value<T>, const T, const T &>;

template <class T>
using return_t =
    std::conditional_t<detail::should_pass_by_value<T>, T, const T &>;

template <bool C> using require = std::enable_if_t<C, int>;
...
template <class T> using range_category_t = iter_category_t<iterator_t<T>>;

template <class T> using range_reference_t = iter_reference_t<iterator_t<T>>;

template <class T> using range_value_t = iter_value_t<iterator_t<T>>;

template <class T> using range_difference_t = iter_difference_t<iterator_t<T>>;

namespace detail {
template <class Category, class T>
struct iterator_of_category
    : std::is_base_of<Category,
                      typename std::iterator_traits<T>::iterator_category> {};
} // namespace detail

template <class T>
struct is_input_iterator
    : detail::iterator_of_category<std::input_iterator_tag, T> {};

template <class T>
struct is_forward_iterator
    : detail::iterator_of_category<std::forward_iterator_tag, T> {};

template <class T>
struct is_bidirectional_iterator
    : detail::iterator_of_category<std::bidirectional_iterator_tag, T> {};

template <class T>
struct is_random_access_iterator
    : detail::iterator_of_category<std::random_access_iterator_tag, T> {};

template <class T> struct is_input_range : is_input_iterator<iterator_t<T>> {};
...
using has_ostream_operator_impl =
    decltype(std::declval<Os>() << std::declval<T>());
...
static constexpr inline struct format_to_fn {
  template <class T> using has_formatter_impl = decltype(formatter<T>{});

  template <class T> static void do_format(std::ostream &os, const T &item) {
...
static constexpr inline struct print_fn {
  template <class... Args> std::ostream &operator()(const Args &...args) const {
...
template <class E> struct error_wrapper { E m_error; };
...
template <class Result> struct to_result {
...
template <class E> struct to_result<result<void, E>> {
  template <class Func, class... Args>
...
constexpr auto to_ok(Func &&func, Args &&...args) -> Result {
  return to_result<Result>{}(std::forward<Func>(func),
                             std::forward<Args>(args)...);
}

template <class Result, class In> constexpr auto to_error(In &&in) -> Result {
...
template <class T, class E> struct result {
  ...
  constexpr result(value_type value)
      : m_storage(std::in_place_type<value_storage>, std::move(value)) {}

  template <class Err,
            class = std::enable_if_t<std::is_constructible_v<error_type>>>
  constexpr result(const error_wrapper<Err> &error)
      : m_storage(std::in_place_type<error_storage>,
                  error_storage{error.m_error}) {}

  template <class Err,
            class = std::enable_if_t<std::is_constructible_v<error_type>>>
  constexpr result(error_wrapper<Err> &&error)
      : m_storage(std::in_place_type<error_storage>,
                  error_storage{std::move(error.m_error)}) {}
  ...
  constexpr value_type &value() & { return std::get<value_storage>(m_storage); }
  ...
  template <class Func,
            class FuncResult = std::invoke_result_t<Func, const T &>,
            class Result = FuncResult>
  constexpr auto and_then(Func &&func) const & -> Result {
    static_assert(
        detail::is_result<FuncResult>::value,
        "and_then: function result type needs to be of `result<T, E>` type");
    return *this //
               ? detail::to_ok<Result>(std::forward<Func>(func), value())
               : detail::to_error<Result>(*this);
  }

  template <class Func, class FuncResult = std::invoke_result_t<Func, T &&>,
            class Result = FuncResult>
  ...
  template <class Func,
            class FuncResult = std::invoke_result_t<Func, const T &>,
            class Result = result<FuncResult, E>>
  ...
  template <class Func,
            ...
            class Result = std::conditional_t<std::is_void_v<FuncResult>,
                                              result<T, E>, FuncResult>>
  constexpr auto or_else(Func &&func) const & -> Result {
    if constexpr (std::is_void_v<FuncResult>) {
      ...
      static_assert(
          detail::is_result<FuncResult>::value,
          "or_else: function result type needs to be of `result<T, E>` type");
      return *this //
                 ? Result{*this}
  ...
  template <class Func,
            class FuncResult = std::invoke_result_t<Func, const E &>,
            class Result = result<T, FuncResult>>
  ...
  constexpr auto transform_error(Func &&func) && -> Result {
    return *this //
               ...
               : Result{error(std::invoke(std::forward<Func>(func),
                                          std::move(*this).error()))};
  }
...
template <class T, class E> struct result<T &, E> {
  ...
  constexpr result(value_type &value)
      : m_storage(std::in_place_type<value_storage>, value) {}
...
template <class E> struct result<void, E> {
  ...
  template <class Err,
            class = std::enable_if_t<std::is_constructible_v<error_type>>>
  constexpr result(const error_wrapper<Err> &error)
      : m_storage(error.m_error) {}

  template <class Err,
            class = std::enable_if_t<std::is_constructible_v<error_type>>>
  constexpr result(error_wrapper<Err> &&error)
      : m_storage(std::move(error.m_error)) {}
  ...
  constexpr const error_type &error() const & { return *m_storage; }

  constexpr error_type &error() & { return *m_storage; }

  constexpr error_type &&error() && { return *std::move(m_storage); }

  constexpr bool has_value() const noexcept { return static_cast<bool>(*this); }

  constexpr bool has_error() const noexcept { return !has_value(); }

  template <class Func, class FuncResult = std::invoke_result_t<Func>,
            class Result = FuncResult>
  constexpr auto and_then(Func &&func) const & -> Result {
    static_assert(
        detail::is_result<FuncResult>::value,
  ...
  template <class Func,
            class FuncResult = std::invoke_result_t<Func, const E &>,
            class Result = result<void, FuncResult>>
  constexpr auto transform_error(Func &&func) const & -> Result {
    return *this //
               ? Result{}
               : Result{error(std::invoke(std::forward<Func>(func), error()))};
...
template <class E> struct formatter<result<void, E>> {
...
template <class T, class E> struct formatter<result<T, E>> {
...
template <class Func, class... Args>
auto try_invoke(Func &&func, Args &&...args) noexcept
    -> result<std::invoke_result_t<Func, Args...>, std::exception_ptr> {
  try {
...
template <class T> struct maybe_underlying_type;

template <class T>
using maybe_underlying_type_t = typename maybe_underlying_type<T>::type;

template <class T> struct maybe_underlying_type<maybe<T>> { using type = T; };

namespace detail {

template <class T> struct is_maybe : std::false_type {};

template <class T> struct is_maybe<maybe<T>> : std::true_type {};
...
template <class T> struct maybe {
  ...
  constexpr const value_type &operator*() const & { return *m_storage; }

  constexpr value_type &operator*() & { return *m_storage; }

  constexpr value_type &&operator*() && { return *std::move(m_storage); }

  constexpr const value_type *operator->() const & { return &**this; }

  constexpr value_type *operator->() & { return &**this; }

  constexpr const value_type &value() const & { return *m_storage; }

  constexpr value_type &value() & { return *m_storage; }

  constexpr value_type &&value() && { return *std::move(m_storage); }
  ...
  constexpr auto and_then(Func &&func) const & -> Result {
    static_assert(
        detail::is_maybe<FuncResult>::value,
        "and_then: function result type needs to be of `maybe<T>` type");
    return *this //
  ...
  constexpr auto or_else(Func &&func) const & -> Result {
    if constexpr (std::is_void_v<FuncResult>) {
      ...
      static_assert(
          detail::is_maybe<FuncResult>::value,
          "or_else: function result type needs to be of `maybe<T>` type");
      return *this //
                 ? Result{*this}
                 : Result{std::invoke(std::forward<Func>(func))};
    }
  ...
  template <class Pred> constexpr auto filter(Pred &&pred) const & -> maybe<T> {
    return std::invoke(std::forward<Pred>(pred), value()) //
               ? maybe<T>{*this}
               : maybe<T>{};
  }

  template <class Pred> constexpr auto filter(Pred &&pred) && -> maybe<T> {
    return std::invoke(std::forward<Pred>(pred), value()) //
...
template <class T> struct maybe<T &> {
  ...
  constexpr value_type &operator*() const { return *m_storage; }

  constexpr value_type *operator->() const { return &**this; }

  constexpr value_type &value() const { return *m_storage; }
...
template <class T> struct formatter<maybe<T>> {
...
template <class Iter> struct reverse_iterator_impl {
...
template <class Iter>
auto make_reverse(Iter b, Iter e)
    -> std::pair<reverse_iterator_t<Iter>, reverse_iterator_t<Iter>> {
  return {reverse_iterator_t<Iter>(e), reverse_iterator_t<Iter>(b)};
}

template <class Iter>
auto make_reverse(std::reverse_iterator<Iter> b, std::reverse_iterator<Iter> e)
    -> std::pair<Iter, Iter> {
...
template <class Iter> class iterator_range {
public:
  using iterator = Iter;
  ...
  constexpr iterator_range(iterator b, size_type n)
      : iterator_range(b, std::next(b, n)) {}
  ...
  template <class Container,
            require<std::is_constructible_v<Container, iterator, iterator>> = 0>
  ...
  template <class It = iterator,
            require<is_random_access_iterator<It>::value> = 0>
  ...
  template <class It = iterator,
            require<is_bidirectional_iterator<It>::value> = 0>
  ...
  template <class Pred> auto take_while(Pred &&pred) const -> iterator_range {
    const iterator found =
        std::find_if_not(begin(), end(), std::forward<Pred>(pred));
    return iterator_range(begin(), found);
  }

  template <class Pred> auto drop_while(Pred &&pred) const -> iterator_range {
    const iterator found =
        std::find_if_not(begin(), end(), std::forward<Pred>(pred));
    return iterator_range(found, end());
  }

  template <class Pred, class It = iterator,
            require<is_bidirectional_iterator<It>::value> = 0>
  ...
  template <class Pred> auto find_if(Pred &&pred) const -> maybe_reference {
    const iterator found =
        std::find_if(begin(), end(), std::forward<Pred>(pred));
...
template <class T> using span = iterator_range<const T *>;

template <class T> using mut_span = iterator_range<T *>;

struct identity {
  template <class T> constexpr T &&operator()(T &&item) const noexcept {
    return std::forward<T>(item);
  }
};

template <class... Args> struct formatter<std::tuple<Args...>> {
...
template <class F, class S> struct formatter<std::pair<F, S>> {
...
private:
  ...
  template <std::size_t I, class... Args,
            std::enable_if_t<(I + 1) < sizeof...(Pipes), int> = 0>
  constexpr auto call(Args &&...args) const
      -> decltype(call<I + 1>(invoke<I>(std::forward<Args>(args)...))) {
...
template <class T> struct is_pipeline : std::false_type {};

template <class... Args>
struct is_pipeline<pipe_t<Args...>> : std::true_type {};

template <class... L, class... R>
constexpr auto operator|=(pipe_t<L...> lhs, pipe_t<R...> rhs)
    -> decltype(pipe(std::move(lhs), std::move(rhs))) {
  return pipe(std::move(lhs), std::move(rhs));
}

template <class T, class... Pipes,
          std::enable_if_t<!is_pipeline<std::decay_t<T>>{}, int> = 0>
constexpr auto operator|=(T &&item, const pipe_t<Pipes...> &p)
    -> decltype(p(std::forward<T>(item))) {
  return p(std::forward<T>(item));
}

template <class... Pipes> struct formatter<pipe_t<Pipes...>> {
...
private:
  ...
  template <class... Pipes>
  constexpr auto from_tuple(std::tuple<Pipes...> tuple) const
      -> pipe_t<Pipes...> {
...
public:
  template <class... Pipes>
  constexpr auto operator()(Pipes &&...pipes) const -> decltype(from_tuple(
      std::tuple_cat(to_tuple(std::forward<Pipes>(pipes))...))) {
    return from_tuple(std::tuple_cat(to_tuple(std::forward<Pipes>(pipes))...));
  }
};

static constexpr inline struct do_all_fn {
  template <class... Funcs> struct impl {
...
static constexpr struct apply_fn {
  template <class Func> struct impl {
    Func m_func;

    template <class T> T &operator()(T &item) const {
  ...
  template <class... Funcs>
  auto operator()(Funcs &&...funcs) const
      -> pipe_t<impl<decltype(do_all(std::forward<Funcs>(funcs)...))>> {
...
template <class T> using iteration_result_t = maybe<T>;
...
template <class T> struct inspect_mixin {
  template <class Func> struct next_function {
    Func m_func;
  ...
  template <class Func> auto inspect(Func &&func) const & -> sequence<T> {
    return sequence<T>{next_function<std::decay_t<Func>>{
        std::forward<Func>(func),
        static_cast<const sequence<T> &>(*this).get_next_function()}};
  }

  template <class Func> auto inspect(Func &&func) && -> sequence<T> {
    return sequence<T>{next_function<std::decay_t<Func>>{
        std::forward<Func>(func),
...
template <class T> struct inspect_indexed_mixin {
  template <class Func> struct next_function {
    Func m_func;
    next_function_t<T> m_next;
  ...
  template <class Func> auto inspect_indexed(Func &&func) && -> sequence<T> {
    return sequence<T>{next_function<std::decay_t<Func>>{
        std::forward<Func>(func),
        static_cast<sequence<T> &&>(*this).get_next_function()}};
  }
...
template <class T> struct transform_mixin {
  template <class Func, class Out> struct next_function {
    Func m_func;
    next_function_t<T> m_next;
...
template <class T> struct transform_indexed_mixin {
  template <class Func, class Out> struct next_function {
    Func m_func;
    next_function_t<T> m_next;
    mutable std::ptrdiff_t m_index = 0;
...
template <class T> struct transform_maybe_mixin {
  template <class Func, class Out> struct next_function {
    Func m_func;
    next_function_t<T> m_next;
...
template <class T> struct transform_maybe_indexed_mixin {
  template <class Func, class Out> struct next_function {
    Func m_func;
    next_function_t<T> m_next;
    ...
    auto operator()() const -> iteration_result_t<Out> {
      while (true) {
        ...
        iteration_result_t<Out> r =
            std::invoke(m_func, m_index++, *std::move(res));
        if (r) {
          return r;
        }
      }
  ...
  template <class Func, class Res = maybe_underlying_type_t<
                            std::invoke_result_t<Func, std::ptrdiff_t, T>>>
...
template <class T> struct filter_mixin {
  template <class Pred> struct next_function {
    Pred m_pred;
    next_function_t<T> m_next;
  ...
  template <class Pred> auto filter(Pred &&pred) const & -> sequence<T> {
    return sequence<T>{next_function<std::decay_t<Pred>>{
        std::forward<Pred>(pred),
        static_cast<const sequence<T> &>(*this).get_next_function()}};
  }

  template <class Pred> auto filter(Pred &&pred) && -> sequence<T> {
    return sequence<T>{next_function<std::decay_t<Pred>>{
        std::forward<Pred>(pred),
        static_cast<sequence<T> &&>(*this).get_next_function()}};
  }
...
template <class T> struct filter_indexed_mixin {
  template <class Pred> struct next_function {
    Pred m_pred;
    next_function_t<T> m_next;
    mutable std::ptrdiff_t m_index = 0;
  ...
  template <class Pred> auto filter_indexed(Pred &&pred) && -> sequence<T> {
    return sequence<T>{next_function<std::decay_t<Pred>>{
        std::forward<Pred>(pred),
        static_cast<sequence<T> &&>(*this).get_next_function()}};
  }
...
template <class T> struct drop_while_mixin {
  template <class Pred> struct next_function {
    Pred m_pred;
    next_function_t<T> m_next;
  ...
  template <class Pred> auto drop_while(Pred &&pred) const & -> sequence<T> {
    return sequence<T>{next_function<std::decay_t<Pred>>{
        std::forward<Pred>(pred),
        static_cast<const sequence<T> &>(*this).get_next_function()}};
  }

  template <class Pred> auto drop_while(Pred &&pred) && -> sequence<T> {
    return sequence<T>{next_function<std::decay_t<Pred>>{
        std::forward<Pred>(pred),
        static_cast<sequence<T> &&>(*this).get_next_function()}};
  }
...
template <class T> struct drop_while_indexed_mixin {
  template <class Pred> struct next_function {
    Pred m_pred;
    next_function_t<T> m_next;
    mutable bool m_init = true;
  ...
  template <class Pred> auto drop_while_indexed(Pred &&pred) && -> sequence<T> {
    return sequence<T>{next_function<std::decay_t<Pred>>{
        std::forward<Pred>(pred),
        static_cast<sequence<T> &&>(*this).get_next_function()}};
  }
...
template <class T> struct take_while_mixin {
  template <class Pred> struct next_function {
    Pred m_pred;
    next_function_t<T> m_next;
  ...
  template <class Pred> auto take_while(Pred &&pred) const & -> sequence<T> {
    return sequence<T>{next_function<std::decay_t<Pred>>{
        std::forward<Pred>(pred),
        static_cast<const sequence<T> &>(*this).get_next_function()}};
  }

  template <class Pred> auto take_while(Pred &&pred) && -> sequence<T> {
    return sequence<T>{next_function<std::decay_t<Pred>>{
        std::forward<Pred>(pred),
        static_cast<sequence<T> &&>(*this).get_next_function()}};
  }
...
template <class T> struct take_while_indexed_mixin {
  template <class Pred> struct next_function {
    Pred m_pred;
    next_function_t<T> m_next;
    mutable std::ptrdiff_t m_index = 0;
  ...
  template <class Pred> auto take_while_indexed(Pred &&pred) && -> sequence<T> {
    return sequence<T>{next_function<std::decay_t<Pred>>{
        std::forward<Pred>(pred),
        static_cast<sequence<T> &&>(*this).get_next_function()}};
  }
...
template <class T> struct drop_mixin {
  ...
  auto drop(std::ptrdiff_t n) const & -> sequence<T> {
    return sequence<T>{next_function{
        n, static_cast<const sequence<T> &>(*this).get_next_function()}};
  }

  auto drop(std::ptrdiff_t n) && -> sequence<T> {
    return sequence<T>{next_function{
        n, static_cast<sequence<T> &&>(*this).get_next_function()}};
  }
};

template <class T> struct take_mixin {
  struct next_function {
    mutable std::ptrdiff_t m_count;
    next_function_t<T> m_next;
...
template <class T> struct step_mixin {
  struct next_function {
    mutable std::ptrdiff_t m_count;
    next_function_t<T> m_next;
    mutable std::ptrdiff_t m_index = 0;
...
template <class T> struct join_mixin {};

template <class T> struct join_mixin<sequence<T>> {
  struct next_function {
  ...
  auto join() const & -> sequence<T> {
    return sequence<T>{next_function{
        static_cast<const sequence<sequence<T>> &>(*this).get_next_function()}};
  }

  auto join() && -> sequence<T> {
    return sequence<T>{next_function{
        static_cast<sequence<sequence<T>> &&>(*this).get_next_function()}};
  }
};

template <class T> struct for_each_mixin {
  template <class Func> void for_each(Func &&func) const & {
    const auto next_function =
        static_cast<const sequence<T> &>(*this).get_next_function();
    while (true) {
...
template <class T> struct for_each_indexed_mixin {
  template <class Func> void for_each_indexed(Func &&func) const & {
    const auto next_function =
        static_cast<const sequence<T> &>(*this).get_next_function();
...
template <class T> struct empty_sequence {
  auto operator()() const -> iteration_result_t<T> { return {}; }
};

template <class To, class From> struct cast_sequence {
...
template <class Iter, class Out> struct view_sequence {
  mutable Iter m_iter;
  Iter m_end;
...
template <class T> struct pointer_proxy {
  T item;

  T *operator->() { return std::addressof(item); }
};
...
template <class T> struct sequence_iterator {
  ...
  sequence_iterator()
      : m_next_fn{}, m_current{},
        m_index{std::numeric_limits<difference_type>::max()} {}

  sequence_iterator(const next_function_type &next_fn)
      : m_next_fn{next_fn}, m_current{m_next_fn()}, m_index{0} {}
  ...
  friend bool operator==(const sequence_iterator &lhs,
                         const sequence_iterator &rhs) {
    return (!lhs.m_current && !rhs.m_current) ||
           (lhs.m_current && rhs.m_current && lhs.m_index == rhs.m_index);
  }
...
struct sequence : inspect_mixin<T>,
  ...
  explicit sequence(next_function_type next_fn)
      : m_next_fn(std::move(next_fn)) {}

  template <class U,
            std::enable_if_t<std::is_constructible_v<reference, U>, int> = 0>
  sequence(const sequence<U> &other)
      : sequence(cast_sequence<reference, U>{other.get_next_function()}) {}

  template <class U,
            std::enable_if_t<std::is_constructible_v<reference, U>, int> = 0>
  sequence(sequence<U> &&other)
      : sequence(
            cast_sequence<reference, U>{std::move(other).get_next_function()}) {
  }

  template <
      class Iter,
      std::enable_if_t<
          std::is_constructible_v<reference, iter_reference_t<Iter>>, int> = 0>
  sequence(Iter b, Iter e) : sequence(view_sequence<Iter, reference>{b, e}) {}

  template <
      class Range, class Iter = iterator_t<Range>,
      std::enable_if_t<
          std::is_constructible_v<reference, iter_reference_t<Iter>>, int> = 0>
  ...
  template <
      class Container,
      std::enable_if_t<std::is_constructible_v<Container, iterator, iterator>,
                       int> = 0>
  operator Container() const {
    return Container{begin(), end()};
  }
  ...
  template <class Pred> auto find_if(Pred pred) const -> maybe<reference> {
  ...
  template <class Output> auto copy(Output out) const -> Output {
  ...
  auto accumulate(Seed seed, BinaryFunc &&func) const -> Seed {
    return std::accumulate(begin(), end(), std::move(seed),
...
struct iota_fn {
  template <class In> struct next_function {
    mutable In m_current;
    auto operator()() const -> iteration_result_t<In> { return m_current++; }
  };

  template <class T> auto operator()(T init) const -> sequence<T> {
...
struct range_fn {
  ...
  template <class T> auto operator()(T lower, T upper) const -> sequence<T> {
    return sequence<T>{next_function<T>{lower, upper}};
  }

  template <class T> auto operator()(T upper) const -> sequence<T> {
    return (*this)(T{}, upper);
  }
};

struct unfold_fn {
  template <class Func, class S, class Out> struct next_function {
    Func m_func;
    mutable S m_state;
  ...
  auto operator()(S state, Func &&func) const -> sequence<Out> {
    return sequence<Out>{next_function<std::decay_t<Func>, S, Out>{
        std::forward<Func>(func), std::move(state)}};
  }
};

struct view_fn {
  template <class Iter, class Out> struct next_function {
    mutable Iter m_iter;
    Iter m_end;
  ...
  auto operator()(Range &&range) const -> sequence<Out> {
    return sequence<Out>{next_function<iterator_t<Range>, Out>{
  ...
  template <class T> auto operator()(sequence<T> &&seq) const -> sequence<T> {
    return seq;
  }
};

struct owning_fn {
  template <class Range, class Iter, class Out> struct next_function {
    std::shared_ptr<Range> m_range;
    mutable Iter m_iter;

    next_function(std::shared_ptr<Range> range)
        : m_range(range), m_iter(std::begin(*m_range)) {}
  ...
  template <class Range, class Out = range_reference_t<Range>>
  auto operator()(Range range) const -> sequence<Out> {
    return sequence<Out>{next_function<Range, iterator_t<Range>, Out>{
  ...
  template <class T> auto operator()(sequence<T> &&s) const -> sequence<T> {
    return s;
  }
};

struct single_fn {
  template <class T> struct next_function {
    T m_value;
    mutable bool m_init = true;
  ...
  template <class T> auto operator()(T value) const -> sequence<const T &> {
...
struct repeat_fn {
  template <class T> struct next_function {
    ...
    auto operator()() const -> iteration_result_t<const T &> { return m_value; }
  };
...
struct concat_fn {
  ...
  template <class T0, class T1, class T2, class T3,
            class Out = std::common_type_t<T0, T1, T2, T3>>
  auto operator()( //
      const sequence<T0> &s0, const sequence<T1> &s1, const sequence<T2> &s2,
      const sequence<T3> &s3) const -> sequence<Out> {
    return (*this)((*this)(s0, s1, s2), s3);
  }

  template <class T0, class T1, class T2,
            class Out = std::common_type_t<T0, T1, T2>>
  auto operator()( //
      const sequence<T0> &s0, const sequence<T1> &s1,
  ...
  auto operator()( //
      const sequence<T0> &s0, const sequence<T1> &s1) const -> sequence<Out> {
    return (*this)(sequence<Out>{s0}, sequence<Out>{s1});
  }

  template <class T>
  auto operator()( //
      const sequence<T> &lhs, const sequence<T> &rhs) const -> sequence<T> {
    return sequence<T>{
        next_function<T>{lhs.get_next_function(), rhs.get_next_function()}};
  }
};
...
struct zip_fn {
  ...
  template <class In0, class In1, class In2, class In3> struct next_function {
    ...
    auto operator()() const
        -> iteration_result_t<std::tuple<In0, In1, In2, In3>> {
  ...
  template <class In0, class In1> struct next_function<In0, In1, void, void> {
    next_function_t<In0> m_next0;
    next_function_t<In1> m_next1;
  ...
  auto operator()( //
      const sequence<T0> &s0, const sequence<T1> &s1,
      const sequence<T2> &s2) const -> sequence<Out> {
    return sequence<Out>{next_function<T0, T1, T2>{s0.get_next_function(),
                                                   s1.get_next_function(),
                                                   s2.get_next_function()}};
  }
  ...
  auto operator()( //
      const sequence<T0> &s0, const sequence<T1> &s1) const -> sequence<Out> {
    return sequence<Out>{
        next_function<T0, T1>{s0.get_next_function(), s1.get_next_function()}};
  }
};
...
struct vec_fn {
  ...
  auto operator()(T head, Tail &&...tail) const -> sequence<const T &> {
    return owning_fn{}(
        std::vector<T>{std::move(head), std::forward<Tail>(tail)...});
  }
};
...
template <class L, class R>
auto operator+(const sequence<L> &lhs, const sequence<R> &rhs)
    -> sequence<std::common_type_t<L, R>> {
