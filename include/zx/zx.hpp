...
struct variant_with_fn
{
    ...
    struct impl
    {
        ...
        friend std::ostream& operator<<(std::ostream& os, const impl& item)
        {
            return os << "(variant_with " << type<T>{} << " " << item.pred << ")";
        }
    };
...
static constexpr inline auto result_of = detail::result_of_fn<str_t<'r', 'e', 's', 'u', 't', 't', '_', 'o', 'f'>>{};
...
template <std::size_t N>
static constexpr inline auto element_at = detail::field_at_fn<N>{};
